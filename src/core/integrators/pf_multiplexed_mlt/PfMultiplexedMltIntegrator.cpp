#include "PfMultiplexedMltIntegrator.hpp"
...
PfMultiplexedMltIntegrator::PfMultiplexedMltIntegrator()
: Integrator(),
  _w(0),
  _h(0),
  _sampler(0xBA5EBA11)
...
void PfMultiplexedMltIntegrator::saveState(OutputStreamHandle &/*out*/)
{
    FAIL("PfMultiplexedMltIntegrator::saveState not supported!");
}

void PfMultiplexedMltIntegrator::loadState(InputStreamHandle &/*in*/)
{
    FAIL("PfMultiplexedMltIntegrator::loadState not supported!");
}

void PfMultiplexedMltIntegrator::traceSamplePool(uint32 taskId, uint32 numSubTasks, uint32 /*threadId*/)
{
...
void PfMultiplexedMltIntegrator::runSampleChain(uint32 taskId, uint32 numSubTasks, uint32 /*threadId*/)
{
    ...
    PfLargeStepTracker *stepTrackers = _subtaskData[taskId].independentEstimator.get();

    PfMultiplexedStats stats(*_stats);
...
void PfMultiplexedMltIntegrator::selectSeedPaths()
{
...
void PfMultiplexedMltIntegrator::computeNormalizationFactor()
{
...
void PfMultiplexedMltIntegrator::setBufferWeights()
{
...
void PfMultiplexedMltIntegrator::fromJson(JsonPtr v, const Scene &/*scene*/)
{
    _settings.fromJson(v);
}

rapidjson::Value PfMultiplexedMltIntegrator::toJson(Allocator &allocator) const
{
    return _settings.toJson(allocator);
}

void PfMultiplexedMltIntegrator::saveOutputs()
{
...
void PfMultiplexedMltIntegrator::prepareForRender(TraceableScene &scene, uint32 seed)
{
    ...
    _stats.reset(new AtomicPfMultiplexedStats(_settings.maxBounces));
    ...
    for (uint32 i = 0; i < ThreadUtils::pool->threadCount(); ++i) {
        _tracers.emplace_back(new PfMultiplexedMltTracer(&scene, _settings, i, _sampler, _imagePyramid.get()));
        _subtaskData.emplace_back();
        _subtaskData.back().independentEstimator.reset(new PfLargeStepTracker[_settings.maxBounces + 1]);
    }
}

void PfMultiplexedMltIntegrator::teardownAfterRender()
{
...
void PfMultiplexedMltIntegrator::startRender(std::function<void()> completionCallback)
{
    ...
    if (!_chainsLaunched) {
        ...
        _group = ThreadUtils::pool->enqueue(
            std::bind(&PfMultiplexedMltIntegrator::traceSamplePool, this, _1, _2, _3),
            _tracers.size(),
            [&, completionCallback]() {
                selectSeedPaths();
        ...
        _group = ThreadUtils::pool->enqueue(
            std::bind(&PfMultiplexedMltIntegrator::runSampleChain, this, _1, _2, _3),
            _tracers.size(),
            [&, completionCallback]() {
...
void PfMultiplexedMltIntegrator::waitForCompletion()
{
    if (_group) {
        _group->wait();
        _group.reset();
...
void PfMultiplexedMltIntegrator::abortRender()
{
    if (_group) {
        _group->abort();
        _group->wait();
