#ifndef PFMULTIPLEXEDMLTINTEGRATOR_HPP_
#define PFMULTIPLEXEDMLTINTEGRATOR_HPP_
...
class PfMultiplexedMltIntegrator : public Integrator
{
    ...
    struct SubtaskData
    {
        std::unique_ptr<PfLargeStepTracker[]> independentEstimator;
        uint32 rangeStart  = 0;
    ...
    std::vector<std::unique_ptr<PfMultiplexedMltTracer>> _tracers;

    bool _chainsLaunched;
    double _luminanceScale;
    std::atomic<uint64> _numSeedPathsTraced;
    std::vector<PfLargeStepTracker> _luminancePerLength;
    std::unique_ptr<PathCandidate[]> _pathCandidates;

    std::unique_ptr<AtomicPfMultiplexedStats> _stats;
...
#endif /* PFMULTIPLEXEDMLTINTEGRATOR_HPP_ */
