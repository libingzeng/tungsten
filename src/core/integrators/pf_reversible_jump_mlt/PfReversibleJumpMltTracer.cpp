#include "PfReversibleJumpMltTracer.hpp"

#include "sampling/UniformSampler.hpp"

namespace Tungsten {

PfReversibleJumpMltTracer::PfReversibleJumpMltTracer(TraceableScene *scene, const PfReversibleJumpMltSettings &settings, uint32 threadId,
...
void PfReversibleJumpMltTracer::tracePaths(
...
void PfReversibleJumpMltTracer::evalSample(PfWritableMetropolisSampler &cameraSampler, PfWritableMetropolisSampler &emitterSampler,
...
void PfReversibleJumpMltTracer::traceCandidatePath(LightPath &cameraPath, LightPath &emitterPath,
...
void PfReversibleJumpMltTracer::startSampleChain(int s, int t, float luminance, UniformSampler &cameraReplaySampler,
        UniformSampler &emitterReplaySampler)
{
    int length = s + t - 1;

    MarkovChain &chain = _chains[length];
    chain. cameraSampler.reset(new PfWritableMetropolisSampler(_settings.gaussianMutation, & cameraReplaySampler, length + 4));
    chain.emitterSampler.reset(new PfWritableMetropolisSampler(_settings.gaussianMutation, &emitterReplaySampler, length + 4));
...
LargeStepTracker PfReversibleJumpMltTracer::runSampleChain(int pathLength, int chainLength,
        MultiplexedStats &stats, float luminanceScale)
{
    MarkovChain &chain = _chains[pathLength];
    PfWritableMetropolisSampler & cameraSampler = *chain. cameraSampler;
    PfWritableMetropolisSampler &emitterSampler = *chain.emitterSampler;
...
#define PFMLT 1
    ...
    for (int iter = 0; iter < chainLength; ++iter) {
        ...
        if(a < 0.000001 && CNT < 5 && CNT < pathLength+2){//default: CNT < 5, 2
            CNT ++;
