#include "PfReversibleJumpMltIntegrator.hpp"
...
PfReversibleJumpMltIntegrator::PfReversibleJumpMltIntegrator()
: Integrator(),
  _w(0),
  _h(0),
  _sampler(0xBA5EBA11)
...
void PfReversibleJumpMltIntegrator::saveState(OutputStreamHandle &/*out*/)
{
    FAIL("PfReversibleJumpMltIntegrator::saveState not supported!");
}

void PfReversibleJumpMltIntegrator::loadState(InputStreamHandle &/*in*/)
{
    FAIL("PfReversibleJumpMltIntegrator::loadState not supported!");
}

void PfReversibleJumpMltIntegrator::traceSamplePool(uint32 taskId, uint32 numSubTasks, uint32 /*threadId*/)
{
...
void PfReversibleJumpMltIntegrator::runSampleChain(uint32 taskId, uint32 numSubTasks, uint32 /*threadId*/)
{
...
void PfReversibleJumpMltIntegrator::selectSeedPaths()
{
...
void PfReversibleJumpMltIntegrator::computeNormalizationFactor()
{
...
void PfReversibleJumpMltIntegrator::setBufferWeights()
{
...
void PfReversibleJumpMltIntegrator::fromJson(JsonPtr v, const Scene &/*scene*/)
{
    _settings.fromJson(v);
}

rapidjson::Value PfReversibleJumpMltIntegrator::toJson(Allocator &allocator) const
{
    return _settings.toJson(allocator);
}

void PfReversibleJumpMltIntegrator::saveOutputs()
{
...
void PfReversibleJumpMltIntegrator::prepareForRender(TraceableScene &scene, uint32 seed)
{
    ...
    for (uint32 i = 0; i < ThreadUtils::pool->threadCount(); ++i) {
        _tracers.emplace_back(new PfReversibleJumpMltTracer(&scene, _settings, i, _sampler, _imagePyramid.get()));
        _subtaskData.emplace_back();
        _subtaskData.back().independentEstimator.reset(new LargeStepTracker[_settings.maxBounces + 1]);
    }
}

void PfReversibleJumpMltIntegrator::teardownAfterRender()
{
...
void PfReversibleJumpMltIntegrator::startRender(std::function<void()> completionCallback)
{
    ...
    if (!_chainsLaunched) {
        ...
        _group = ThreadUtils::pool->enqueue(
            std::bind(&PfReversibleJumpMltIntegrator::traceSamplePool, this, _1, _2, _3),
            _tracers.size(),
            [&, completionCallback]() {
                selectSeedPaths();
        ...
        _group = ThreadUtils::pool->enqueue(
            std::bind(&PfReversibleJumpMltIntegrator::runSampleChain, this, _1, _2, _3),
            _tracers.size(),
            [&, completionCallback]() {
...
void PfReversibleJumpMltIntegrator::waitForCompletion()
{
    if (_group) {
        _group->wait();
        _group.reset();
...
void PfReversibleJumpMltIntegrator::abortRender()
{
    if (_group) {
        _group->abort();
        _group->wait();
